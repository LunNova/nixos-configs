...
if [ -v 1 ] && [ "$1" == "all" ]; then
    nix flake show --json | jq '.nixosConfigurations | keys[] as $k | select(.[$k].type=="nixos-configuration") | $k' -cr | xargs -tI{} nix build .#nixosConfigurations.{}.config.system.build.toplevel
else
