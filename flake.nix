{
  ...
  outputs = flake-args:
    let
      perSystem = import ./per-system.nix { inherit flake-args; };
      inherit (flake-args) self;
    in
    {
      inherit flake-args perSystem;

      lib = import ./lib { bootstrapLib = flake-args.nixpkgs.lib; };
      assets = import ./assets;
      overlays.default = import ./overlay.nix { inherit flake-args; };
      overlay = self.overlays.default; # deprecated alias, TODO: remove in 2023
      localPackagesForPkgs = pkgs: import ./packages { inherit pkgs flake-args; };
      ...
      deploy.nodes.router = {
        ...
        profiles.system = {
          sshUser = "lun";
          user = "root";
          path = flake-args.deploy-rs.lib.x86_64-linux.activate.nixos self.nixosConfigurations.router-nixos;
        };
      };
    } // flake-args.flake-utils.lib.eachDefaultSystem perSystem;
}
